...
- **변경사항**:
  - HOME 없을 경우 숙소 추천 섹션 강화:
    - chat에서 예산/위치 선호도 추론
    - 여행지 중심부 또는 교통 허브 근처 추천
    - 숙소 정보 Google Maps로 조회
    - 추천 이유 간단히 명시 (내부 로직용, 응답에는 미포함)
- **상태**: 대기 중
...
- **변경사항**:
  - 숙소 출발/귀가 예외 케이스 명시:
    - chat에서 "공항에서 출발", "역에서 시작" 등 명시적 요청 시 예외 허용
    - rule에서 "Day 1: XX에서 출발" 등 명시 시 우선 적용
    - 단, 명시적 요청 없으면 기본적으로 숙소 왕복
- **상태**: 대기 중
