...
class ItineraryGeneratorService3:
    """V2 일정 생성 서비스 (Gemini 중심)"""

    def __init__(self):
        """Gemini 클라이언트 초기화 (Version 3: Gemini 3 Pro Preview)"""
        self.client = genai.Client(api_key=settings3.google_api_key)
        self.model_name = settings3.gemini3_model_name
        logger.info(f"ItineraryGeneratorService3 initialized with {self.model_name} (Gemini 3 Pro Preview) and Google Maps grounding")
        ...
        try:
            ...
            response = self.client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    temperature=settings3.gemini3_temperature,  # 0.5 - Gemini 3 Pro는 낮은 temperature에서도 뛰어난 추론 능력 발휘
                    # Note: response_mime_type="application/json" is not supported with Google Maps tool
                    # Gemini 3 Pro Preview는 향상된 멀티모달 이해와 SOTA 추론 능력 제공
                    tools=[
...
**절대적 필수 사항**:
- **기본값**: 하루 일정은 **반드시 10-12시간** 정도가 되어야 합니다
- **계산 방법**: 첫 visit의 arrival ~ 마지막 visit의 departure
- **예외 처리**: 사용자 대화(chat)나 규칙(rule)에 다른 요청이 있으면 그에 따르되, **최소 일정 길이는 반드시 확보**하세요
  - "여유롭게", "느긋하게", "천천히" → **8-10시간** (장소 수는 줄이되, 체류시간을 늘려서 일정 길이 확보)
  - "알차게", "많이 보고 싶어", "빡빡하게" → **12-14시간**
  - 명시적 요청 없음 → **10-12시간 (기본값)**

**중요**: "여유롭게"는 **일정을 짧게 하는 것이 아니라**, 장소를 적게 방문하고 각 장소에서 충분한 시간을 보내는 것을 의미합니다.

**예시**:
- ❌ **잘못된 해석**: "여유롭게" → 09:00-14:00 (5시간) - 너무 짧음!
- ⭕ **올바른 해석**: "여유롭게" → 09:00-17:00 (8시간), 장소 3-4곳만 방문, 각 장소 체류시간 2-3시간

**검증 방법**:
- 각 day에서: (마지막 visit의 departure) - (첫 visit의 arrival) = 일정 길이
- 기본: 10-12시간, 여유: 8-10시간, 알차게: 12-14시간
- 이 기준을 **반드시** 충족해야 합니다

**절대 규칙**:
- 어떤 상황에서도 하루 일정이 8시간 미만이 되어서는 안 됩니다 (마지막 날 공항 이동 등 특수한 경우 제외)
- 일정 길이가 부족하면 체류시간을 늘리거나 장소를 추가하여 길이를 맞추세요
...
# 싱글톤 인스턴스
itinerary_generator_service3 = ItineraryGeneratorService3()
