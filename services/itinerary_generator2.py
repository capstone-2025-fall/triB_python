...
### 3. 🔴 장소 선택 및 필수 방문 장소 포함 (Priority 1-A, 1-D)
- 위의 "고려 중인 장소 목록 (places)"에서 적절한 장소를 우선적으로 선택하세요
- **필수 방문 장소 (must_visit)는 반드시 100% 일정에 포함하세요**
  - 일정이 부족하면 다른 장소를 제거하더라도 must_visit는 유지하세요
  - must_visit 장소의 운영시간이 맞지 않으면 다른 날짜로 조정하세요
  - 절대 생략할 수 없습니다
...
### 7. 🔴 일정 기간 준수 (Priority 1-C)
여행 기간 정보는 상단의 "여행 기간" 섹션에 명시되어 있습니다.

**규칙**:
1. 정확히 요청된 일수만큼의 day 항목을 생성하세요
2. 각 day의 날짜는 start_date부터 순차적으로 증가해야 합니다
3. 하루를 추가하거나 빼지 마세요
4. day 필드는 1부터 시작하여 순차적으로 증가합니다 (1, 2, 3, ...)

**예시**:
- 여행 기간이 3일이고 start_date가 2025-10-15이면
  - Day 1: 2025-10-15 (수요일)
  - Day 2: 2025-10-16 (목요일)
  - Day 3: 2025-10-17 (금요일)
- itinerary 배열에는 정확히 3개의 day 객체가 있어야 함

### 8. 🔴 규칙 준수 (Priority 1-B - 최우선)
- **"반드시 지켜야 할 규칙 (rule)"의 모든 항목을 100% 일정에 반영하세요**
- **우선순위**: 규칙(rule) > 숙소 왕복 > 기본 패턴
- 각 규칙을 정확히 해석하고 일정에 반영하세요:
  - 시간 제약: "11시 기상" → 첫 방문은 11:00 이후
  - 활동 요구: "점심은 현지 맛집" → 점심시간에 RESTAURANT 방문
  - 일정 지정: "둘째날은 유니버설 하루 종일이지?" → Day 2는 유니버설 스튜디오만 포함
  - 이동 제약: "마지막날 공항으로" → 마지막날은 숙소 대신 공항으로 종료
- 규칙 간 충돌 시 먼저 나온 규칙을 우선하되, 사용자의 안전과 편의를 최우선으로 고려하세요
