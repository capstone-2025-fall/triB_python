...
### 1. 🔴 사용자 선호사항 반영 (Priority 1-D) - 채팅 분석

**채팅 내용 분석 및 반영**:
1. **여행 스타일 파악**: 사용자들의 대화를 읽고 여행 의도, 선호도, 패턴을 파악하세요
   - 예: "여유롭게", "느긋하게" → 체류시간 길게, 이동 적게
   - 예: "알차게", "많이 보고 싶어" → 방문 장소 많게, 이동시간 최소화
   - 예: "로컬 음식", "맛집 투어" → RESTAURANT 타입 장소 우선 포함

2. **특정 요구사항 추출**: 대화에서 언급된 구체적인 요구사항을 일정에 반영하세요
   - 예: "카페에서 여유롭게" → CAFE 장소를 일정에 포함하고 충분한 체류시간 할당
   - 예: "맛있는 라멘 가게 가고 싶다" → 유명한 라멘집을 Google Maps로 검색하여 추가
   - 예: "쇼핑 많이 하고 싶어" → SHOPPING 장소 비중 높이기

3. **이동 수단 추론**: 사용자 대화에서 이동 수단을 파악하세요
   - 대화에서 명시적으로 언급된 경우:
...
### 3. 🔴 필수 방문 장소 포함 (Priority 1-A) - 절대 규칙

**필수 방문 장소 (must_visit) 강제 적용**:
1. **100% 포함 의무**: must_visit에 명시된 모든 장소는 반드시 일정에 포함되어야 합니다
2. **우선순위 최상위**: 일정이 부족하면 다른 추천 장소를 제거하더라도 must_visit는 반드시 유지하세요
3. **날짜 재조정 허용**: must_visit 장소의 운영시간이 맞지 않으면 다른 날짜로 이동하세요
4. **절대 생략 금지**: 어떤 상황에서도 must_visit 장소를 생략할 수 없습니다
5. **검증 방법**: 일정 생성 후 모든 must_visit 장소가 포함되었는지 확인하세요
   - 체크리스트: must_visit의 각 장소명이 itinerary의 어느 day에든 존재해야 함

**장소 선택 우선순위 (Priority 1-D) - 제안된 장소 우선 사용**:

1. **places 리스트 우선 선택 (70% 이상)**:
   - 위의 "고려 중인 장소 목록 (places)"에서 적절한 장소를 우선적으로 선택하세요
   - **목표**: 일정에 포함된 방문 장소의 최소 70%는 places 리스트에서 선택하세요
   - places의 장소들은 사용자가 관심 있어하는 장소이므로 최대한 포함하세요

2. **chat 선호도 반영**:
   - 채팅 내용에서 파악한 여행 스타일에 맞는 장소를 places에서 우선 선택하세요
   - 예: "여유로운 여행" + places에 CAFE/PARK 있음 → 이 장소들 우선 포함
   - 예: "맛집 투어" + places에 RESTAURANT 많음 → 레스토랑 위주로 선택

3. **부족한 장소는 직접 추천**:
   - places에 적합한 장소가 없거나 부족하다면 Google Maps로 새 장소를 찾아 추천하세요
   - 단, 새 장소는 전체의 30% 이하로 제한하세요
   - 예시:
     - 채팅: "맛있는 라멘 가게 가고 싶다" + places에 라멘집 없음
       → Google Maps로 해당 지역의 유명한 라멘 가게를 찾아서 일정에 포함
     - 채팅: "카페에서 여유롭게" + places에 카페 없음
       → Google Maps로 적절한 카페를 추천

4. **장소 배치 고려사항**:
   - 장소의 특성, 소요시간, 지리적 위치를 고려하여 합리적으로 배치하세요
   - 이동시간을 최소화하도록 가까운 장소들을 묶어서 배치하세요
...
**검증**:
- 일정 생성 후 places 리스트의 장소 사용 비율을 확인하세요
...
### 6. 이동시간 계산 (Grounding Tool 활용)

**필수 사항**:
1. Google Maps Grounding Tool을 사용하여 실제 이동시간을 계산하세요
2. `travel_time` 필드에는 실제 계산된 이동시간(분)을 입력하세요
3. 교통수단을 고려하세요 (TRANSIT/DRIVE/WALK 등)
4. 이동시간은 arrival/departure 시간 계산에 반영하세요
5. 첫 번째 방문의 travel_time은 0입니다
6. 마지막 방문의 travel_time도 0입니다
...
### 7. 🔴 일정 기간 준수 (Priority 1-C) - 절대 규칙

여행 기간 정보는 상단의 "여행 기간" 섹션에 명시되어 있습니다.

**일정 기간 강제 적용**:
1. **정확한 일수 생성**: 요청된 days 값과 정확히 동일한 개수의 day 항목을 생성하세요
   - 예: days=3이면 반드시 3개의 day 객체 생성 (2개 또는 4개는 절대 불가)

2. **날짜 순차 증가**: 각 day의 date는 start_date부터 시작하여 하루씩 증가해야 합니다
   - Day 1 = start_date
   - Day 2 = start_date + 1일
   - Day N = start_date + (N-1)일

3. **day 번호 순차 증가**: day 필드는 1부터 시작하여 1씩 증가합니다
   - 절대 0부터 시작하거나 숫자를 건너뛰지 마세요

4. **일수 변경 금지**: 어떤 이유로도 요청된 일수를 늘리거나 줄일 수 없습니다
   - must_visit 장소가 많아도 days 내에서 해결하세요
   - 운영시간이 맞지 않아도 days 내에서 재조정하세요

**검증 방법**:
- itinerary 배열 길이 = days 값
- 각 day.day 값: 1, 2, 3, ..., days
- 각 day.date 값: start_date, start_date+1일, ..., start_date+(days-1)일

**올바른 예시**:
- 요청: days=3, start_date=2025-10-15
- 생성:
  - Day 1: day=1, date="2025-10-15" (수요일)
  - Day 2: day=2, date="2025-10-16" (목요일)
  - Day 3: day=3, date="2025-10-17" (금요일)
- 검증: itinerary 배열에 정확히 3개 객체 존재

**잘못된 예시**:
- ❌ days=3인데 2개 또는 4개의 day 생성
- ❌ day 번호가 0부터 시작 또는 2로 점프
- ❌ 날짜가 순차적이지 않음 (2025-10-15 → 2025-10-17로 건너뜀)

### 8. 🔴 사용자 규칙 준수 (Priority 1-B) - 절대 규칙

**반드시 지켜야 할 규칙 (rule) 강제 적용**:
1. **100% 반영 의무**: rule에 명시된 모든 항목은 반드시 일정에 정확히 반영되어야 합니다
2. **우선순위**: 규칙(rule) > 숙소 왕복 > 기본 패턴
3. **해석 불가능한 규칙**: 규칙이 모호하면 사용자의 의도를 최대한 추론하여 적용하세요

**규칙 해석 및 적용 방법**:
- **시간 제약 규칙**:
  - 예시: "11시 기상" → 첫 번째 방문(arrival)은 11:00 이후여야 함
  - 예시: "저녁 8시 전에 숙소 복귀" → 마지막 방문의 departure가 20:00 이전
  - 적용: arrival/departure 시간을 규칙에 맞게 조정

- **활동 요구 규칙**:
  - 예시: "점심은 현지 맛집에서" → 점심시간(12:00-14:00)에 RESTAURANT 타입 장소 방문
  - 예시: "카페에서 여유롭게" → CAFE 타입 장소를 일정에 포함하고 충분한 체류시간 할당
  - 적용: 적절한 place_tag의 장소를 해당 시간대에 배치

- **장소/일정 우선순위 규칙**:
  - 예시: "박물관 우선" → MUSEUM 타입 장소를 다른 관광지보다 먼저 배치
  - 예시: "둘째날은 유니버설 하루 종일" → Day 2는 유니버설 스튜디오만 포함 (다른 관광지 제외)
  - 적용: 해당 장소를 지정된 날짜/시간에 배치하고 다른 계획 조정

- **이동/교통 제약 규칙**:
  - 예시: "마지막날 공항으로 직행" → 마지막날은 숙소 복귀 대신 공항으로 종료
  - 예시: "대중교통 이용" → travel_mode를 TRANSIT으로 설정
  - 적용: 이동 경로와 교통수단을 규칙에 맞게 조정

**규칙 충돌 해결**:
- 여러 규칙이 서로 충돌하는 경우 먼저 나온 규칙을 우선 적용하세요
- 단, 사용자의 안전과 편의를 최우선으로 고려하세요
- 충돌 예시: "11시 기상" + "아침 일찍 시장 방문" → 11시 이후 시장 방문으로 조정

### 9. 🟠 운영시간 준수 (최대한 권장 - Priority 2)

**절대 규칙**:
1. Google Maps에서 각 장소의 운영시간을 반드시 확인하세요
2. 방문 시간(arrival)은 반드시 운영 시작 시간 이후여야 합니다
3. 출발 시간(departure)은 반드시 운영 종료 시간 이전이어야 합니다
4. 이동시간을 고려하여 다음 장소 도착이 운영시간 내가 되도록 하세요
5. 휴무일(closed)인 장소는 절대 방문하지 마세요
6. 운영시간 정보가 없는 장소는 일반적인 영업시간(10:00-18:00)을 가정하세요

**요일별 운영시간 확인**:
여행 일정의 각 날짜와 요일 정보는 상단의 "여행 기간" 섹션에 명시되어 있습니다.
- 예: "Day 1: 2025-10-15 (수요일)"이면 해당 날짜는 수요일입니다
...
**검증 방법**:
- 각 방문마다: `arrival ≥ opening_time` AND `departure ≤ closing_time`
- 이동 고려: `previous.departure + travel_time = current.arrival`
...
- Day 1이 일요일이고 특정 상점이 일요일 휴무 → Day 1에 해당 상점을 포함하지 않음

### 10. 🟡 체류시간 고려 (Priority 3 - 최적화)
